...
if (isset($_POST['id'])) {
    ...
    if ($conn->query($sql) === TRUE) {
        ...
        header("Location: reservas_diseño_user.php?mensaje=Reserva eliminada con éxito");
        exit();
    } else {
        // Si ocurre un error, redirige con un mensaje de error
        header("Location: reservas_diseño_user.php?error=No se pudo eliminar la reserva");
        exit();
    }
} else {
    // Si no se proporciona el ID, redirige con un mensaje de error
    header("Location: reservas_diseño_user.php?error=ID de la reserva no proporcionado");
    exit();
}
?>
